...
$(function () {
    ...
    buttons: [
      ...
      },
      {
        ...
        click: async function () {
          ...
          // 🔽 kouji_number が空なら、editingId の order から補完
  ...
  $("#openModal").on("click", function () {
    ...
    setTimeout(() => {
      ...
      $(".ui-dialog-buttonpane button:contains('欠番にする')").hide();   // ← ここ追加（新規では非表示）
    }, 10);
  });
});
...
if (order.missing === 1) {
  koujiNumberCell += "（欠番）";   // ← 欠番なら文字を追加
    ...
    tr.innerHTML = `
      <td>${koujiNumberCell}</td>   <!-- ← ここを修正 -->
      <td>${order.kouji_supplier || ""}</td>
      <td>${order.kouji_kenmei || ""}</td>
      <td>${order.kaichou || ""}</td>
      <td>${order.shachou || ""}</td>
      <td>${order.torishimariyaku || ""}</td>
      <td>${order.soumu || ""}</td>
      <td>${order.buchou || ""}</td>
      <td>${order.hatchuusha || ""}</td>
    `;
